...
class Config:
    ...
    def get_model_path(cls, transform_method=None):
        ...
        return f"{cls.MODEL_DIR}/bearing_fault_cnn_11class_{method}.pth"
    
    @classmethod
    def get_full_model_path(cls, transform_method=None):
        """获取完整模型文件路径"""
        method = transform_method or cls.TRANSFORM_METHOD
        return f"{cls.MODEL_DIR}/bearing_fault_cnn_11class_{method}_full.pth"
    
    @classmethod
    def get_model_state_path(cls):
        """根据TRANSFORM_METHOD生成模型状态文件路径"""
        return f'model/bearing_fault_cnn_11class_{cls.TRANSFORM_METHOD}.pth'
    
    @classmethod
    def get_confusion_matrix_path(cls):
        """根据TRANSFORM_METHOD生成混淆矩阵图片路径"""
        return f'imgs/confusion_matrix_11class_{cls.TRANSFORM_METHOD}.png'
    
    @classmethod
    def get_training_history_path(cls):
        """根据TRANSFORM_METHOD生成训练历史图片路径"""
        return f'imgs/bearing_cnn_training_history_11class_{cls.TRANSFORM_METHOD}.png'
